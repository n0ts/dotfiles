...
;; Version: $Id: sql-completion.el,v 1.1.1.1 2007-05-27 07:29:38 n0ts Exp $
;; Keywords: 
...
(defun sql-mysql-build-databases ()
  (let (sql)
    (dolist (db sql-mysql-update-databases)
      (dolist (tbl (cdr (mysql-shell-query "show tables" db)))
        (setq sql (concat sql (format "select '%s.%s' as '<table>';desc `%s`.`%s`;" db (car tbl) db (car tbl))))))
    (mysql-query-background sql (mysql-connect)
                            'sql-mysql-parse-struct)))

(defun sql-mysql-parse-struct ()
  (let ((output (mysql-output mysql-process))
        tabname tmp)
    (setq output
          (mapcar (lambda (table)
                    (setq table
                    ...
                    (cons (car table)
                          (mapcar (lambda (field)
                                  ...
                                  (split-string (cadr table) "\n" t))))
                  (split-string output "<table>\n" t)))
    (dolist (table output)
      (setq tabname (split-string (car table) "\\.")
            db (sql-add-to-alist 'sql-mysql-schema (car tabname)))
      (add-to-list 'sql-mysql-tables (cadr tabname))
      (mapc (lambda (field) (add-to-list 'sql-mysql-fields field)) (cdr table))
      (setcdr db
              (cons (cons (cadr tabname) (cdr table))
                    (if (setq tmp (assoc (cadr tabname) (cdr db)))
                        (delete tmp (cdr db))
                      (cdr db)))))
